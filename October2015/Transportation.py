...
class LightRailApp:
    ...
    def nextTrain(self, leave, stop, north):
        ...
        if (first != last) and ((first < last) ^ (normLeave < first) ^ (normLeave < last)):
            answer = first + offset
            if answer >= MINUTES_IN_DAY:
                answer -= MINUTES_IN_DAY
            return answer
        ...
        if trainLeave > normLeave:
            crossMidnight = False
            while not crossMidnight:
                trainLeave += intervalPeriods[i]
                if trainLeave >= MINUTES_IN_DAY:
                    trainLeave -= MINUTES_IN_DAY
                    crossMidnight = True
                    if i + 1 < len(self.intervalStarts) and trainLeave >= self.intervalStarts[i + 1]:
                        i += 1
                else:
                    if i + 1 < len(self.intervalStarts) and trainLeave >= self.intervalStarts[i + 1] and self.intervalStarts[i + 1] > self.intervalStarts[i]:
                        i += 1
        while trainLeave < normLeave:
            trainLeave += intervalPeriods[i]
            if i + 1 < len(self.intervalStarts) and trainLeave >= self.intervalStarts[i + 1]:
                i += 1
        answer = trainLeave + offset
        if answer > MINUTES_IN_DAY:
            answer -= MINUTES_IN_DAY
        return answer
...
if __name__ == "__main__":
    ...
    northFirst, northLast = [toMinutes(v) for v in input.readline().strip().split(" ")]
    southFirst, southLast = [toMinutes(v) for v in input.readline().strip().split(" ")]
    ...
    for i in range(queryCount):
        parts = input.readline().strip().split(" ")
        print(formatMinutes(app.nextTrain(toMinutes(parts[0]),int(parts[1]), parts[2] == 'N')))
